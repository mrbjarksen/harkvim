local funcs = require 'harkvim.functions'
...
return {
  ...
  basic = function ()
    ...
    -- Map back/forwards buttons on mouse
    map('', '<X1Mouse>', '<C-O>')
    map('', '<X2Mouse>', '<C-I>')
  end,
  ...
  diagnostics = function ()
    ...
    map('n', '<Leader>dl', funcs.toggle_diag_virt_lines, { desc = "Toggle virtual lines for diagnostics"  })
  end,

  lsp = function (bufnr)
    map('n', 'gd', vim.lsp.buf.definition,  { buffer = bufnr, desc = "Go to definition" })
    map('n', 'gD', vim.lsp.buf.declaration, { buffer = bufnr, desc = "Go to decleration" })
  end,
